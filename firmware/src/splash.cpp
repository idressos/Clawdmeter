...
// 20×20 grid upscaled by the largest integer factor that still fits the
// panel's smaller axis. 2.16 panel (480×480) → 24× = 480×480 (fills).
// 1.8 panel (368×448) → 18× = 360×360, centred in the 368×448 container.
#define GRID         20
#define CELL_X       (LCD_WIDTH  / GRID)
#define CELL_Y       (LCD_HEIGHT / GRID)
#define CELL         ((CELL_X < CELL_Y) ? CELL_X : CELL_Y)
